...
export default function AdminOrdersPage() {
  ...
  const updateOrderMutation = useMutation({
    ...
    onError: (error: any) => {
      toast.error(error.response?.data?.message || (language === 'ar' ? 'فشل في تحديث حالة الطلب' : 'Failed to update order status'));
    },
  });
  ...
  const statusOptions = [
    { value: '', label: language === 'ar' ? 'جميع الطلبات' : 'All Orders' },
            ...
            <div className={`flex items-center ${isRTL ? 'space-x-reverse' : ''} space-x-3`}>
              ...
              <div>
                ...
                <p className="text-gray-600 dark:text-gray-400">
                  {language === 'ar' ? 'إدارة طلبات العملاء وتتبع التسليم' : 'Manage customer orders and track deliveries'}
                </p>
              </div>
            </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
            ...
            <div className="text-sm text-gray-500 dark:text-gray-400">
              {language === 'ar' ? `عرض ${orders.length} طلب` : `Showing ${orders.length} orders`}
            </div>
          </div>
        </div>
